...
def gettask():
	...
	dfPriority = pd.read_csv('DWAEvalCounts.csv', sep=',')
	...
	while task_set==False:
		...
		if tries!=5:
			...
			dwas_ratedten = [rating.dwa for rating in query_ten_done if\
			 (10+dfPriority[dfPriority['DWA Title']==rating.dwa]['remainingNeedCount']-query_count_done[rating.dwa])<=0]
		...
		elif (dwa_times_done_count >= 10 + extra_margin) & (outFlag != 1): 
			tries = tries +1 
			continue
